...
function loadBinding() {
    if (!binding) {
        binding = require('bindings')('hidhandle.node');
    }
}

//This class is a wrapper for `binding.HANDLE` class
function HANDLE() {
    ...
    */
    ...
    this._raw = new (Function.prototype.bind.apply(binding.HANDLE,
        thisPlusArgs))();
    ...
    /* Now we have `this._raw` Object from which we need to
        ...
        `this._raw`
    */
    for (var i in binding.HANDLE.prototype)
        this[i] = binding.HANDLE.prototype[i].bind(this._raw);

    /* We are now done inheriting from `binding.HANDLE` and EventEmitter.

        Now upon adding a new listener for "data" events, we start
        polling the HANDLE device using `read(...)`
...
//Don't inherit from `binding.HANDLE`; that's done above already!
...
HANDLE.prototype.pause = function pause() {
    this._paused = true;
};

HANDLE.prototype.read = function read(callback) {
    if (this._closed) {
        throw new Error('Unable to read from a closed HANDLE device');
    } else {
        return this._raw.read(callback);
    }
};

HANDLE.prototype.resume = function resume() {
    var self = this;
...
function showdevices() {
    loadBinding();
    return binding.devices.apply(HANDLE, arguments);
}

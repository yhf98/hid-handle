 ...
 * @LastEditTime: 2023-03-18 10:51:54
...
Value hmiAddObjJs(const CallbackInfo &info)
{
    ...
    std::vector<obj_attr_t> vec_obj_attr_t;
    ...
    for (uint32_t i = 0; i < objAttrTs.Length(); i++)
    {
        ...
        std::string obj_data = objAttrT.Get("obj_data").As<String>();
        strcpy_s(para.obj_data, obj_data.c_str());

        // obj_reserve[MAX_RESERVE_LEN]; 保留字
        std::string obj_reserve = objAttrT.Get("obj_reserve").As<String>();

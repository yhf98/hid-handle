...
int init(void)
{
	...
	// hid_write_file("./res/menu_4.png", "menu_4.png", TRAN_TYPE_PNG);
	Sleep(1);
	// hid_write_file("./res/00.jpg", "00.jpg", TRAN_TYPE_JPG);
	Sleep(1);
	// hid_write_file("./res/4.mp4", "4.mp4", TRAN_TYPE_VIDEO);
	Sleep(1);

	sprintf((char *const)writebuff, "%s", "sdfsdfaddfdsfwfeerrhgr46757278713rrer2333sfjasdjfoasifjasoidfjasiiodfjasof");

	hid_write_buff(writebuff, 8192, TRAN_TYPE_WEBSITE);
	...
	while (1)
	{
		ret = hid_io_control(CMD_SHOW_PNG, "xxxDDD.png", control_buff, 24);
		switch (ret)
		{
		case SYS_NO_FILE:
		{

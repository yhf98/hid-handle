...
int hmi_page_manage_init(void)
{
	...
	if (page_head == NULL) {
		log_error("invalid elem head parameter.\n");
		return -1;
	}
...
int hmi_page_add_elem(hmi_page_t *page,obj_attr_t para)
{
	...
	if (page->page_elem_num >= MAX_PAGE_ELEM_NUM) {
		log_error("Exceeded the maximum number of elements.\n");
		return -1;
	}

	elem_obj = Hmi_elem_create(para);
	if(elem_obj == NULL){
		log_error("elem create error.\n");
		return -1;
	}
...
int hmi_page_del_elem(hmi_page_t *page , hmi_element_t *obj)
{
	...
	if(page == NULL){
		log_error("page invalid page.\n");
		return -1;
	}
	if(obj == NULL){
		log_error("obj invalid page.\n");
		return -1;
	}
...
hmi_element_t* hmi_page_get_elem_by_id(hmi_page_t* page, unsigned int elem_id)
{
	...
	if (page->page_elem_head == NULL) {
		log_error("invalid elem list tail para.\n");
		return 0;
	}
...
int hmi_page_trav_elem(hmi_page_t *page)
{
	...
	slist_for_each(node, (page->page_elem_head))
	{
		...
		if(elem){
			...
			log_info("elem_color_r=%d\n",elem->elem_attr.obj_r);
			log_info("elem_color_g=%d\n",elem->elem_attr.obj_g);
			log_info("elem_color_b=%d\n",elem->elem_attr.obj_b);
			log_info("elem_action=%d\n",elem->elem_attr.obj_action);
			log_info("==================================\n");
			#endif
		}
	}
...
int hmi_create_obj_test(void)
{
	...
	memcpy(&para.obj_name,"股票",strlen("股票"));
	...
	len = strlen("https://stock-f-reminr-server-aybofoxbug.us-west-1.fcapp.run/?");


	memcpy(&para.obj_url,"https://stock-f-reminr-server-aybofoxbug.us-west-1.fcapp.run/?",len);
	memcpy(&para.obj_field,"price",strlen("price"));
	...
	sprintf((para.obj_data), "%s","00.mp4");
